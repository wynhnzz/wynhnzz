...
OUT = os.path.join(HERE, "..", "generated", "info-card.svg")
...
ROWS = [
    ("host",),
    ("sec", "Profile & Education"),
    ("kv", "Name", "Wayah Hendra"),
    ("kv", "Education", "Telkom University"),
    ("kv", "Role", "Fullstack Developer"),
    ("kv", "Portfolio", "https://wyahhndraa.com"),
    ("kv", "LinkedIn", "https://linkedin.com/in/wayahhendra"),
    ("sec", "Core Development & Architecture Stack"),
    ("kv", "Languages", "HTML, CSS, JS/TS, Go, PHP, C/C++, C#, SQL, Py, Java, Rust"),
    ("kv", "Frontend", "React, Next.js, React Native, Tailwind CSS, Bootstrap"),
    ("kv", "Backend", "Node.js, Express.js, NestJS, Socket.IO"),
    ("kv", "API & Auth", "REST API, JWT Authentication, OpenAPI (Swagger)"),
    ("kv", "Desktop/Win", "Electron, WPF, WinForms, DirectShow, Win32, COM, Reg"),
    ("kv", "Databases/ORM", "MySQL, MongoDB, Redis, Prisma ORM"),
    ("kv", "Cloud/Storage", "AWS S3, Cloudinary, Cloudflare R2"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">wynhnzz@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">wynhnzz</tspan><tspan fill="{MUTED}">@</tspan>'
        ...
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" fill="{SECTION}" font-size="12" font-weight="700">'
                 f'&#8212; {title}</text>'
                 f'<line x1="{KEY_X + 16 + len(row[1])*7.2:.1f}" y1="{y-4:.1f}" x2="{W-PAD}" y2="{y-4:.1f}" '
                 f'stroke="{FRAME}" stroke-opacity="0.8"/>')
    elif kind == "kv":
        key, val_raw = esc(row[1]), row[2]
        if val_raw.startswith("http://") or val_raw.startswith("https://"):
            url = esc(val_raw)
            val = esc(val_raw)
            inner = (f'<text x="{KEY_X}" y="{y:.1f}" fill="{KEY}" font-size="12" font-weight="700">{key}</text>'
                     f'<a href="{url}" target="_blank">'
                     f'<text x="{VAL_X}" y="{y:.1f}" fill="{ACCENT}" font-size="12" text-decoration="underline">{val}</text>'
                     f'</a>')
        else:
            val = esc(val_raw)
            inner = (f'<text x="{KEY_X}" y="{y:.1f}" fill="{KEY}" font-size="12" font-weight="700">{key}</text>'
        ...
        txt_raw = row[1]
        if txt_raw.startswith("http://") or txt_raw.startswith("https://"):
            url = esc(txt_raw)
            txt = esc(txt_raw)
            inner = (f'<circle cx="{KEY_X+3}" cy="{y-4:.1f}" r="2.5" fill="{GREEN}"/>'
                     f'<a href="{url}" target="_blank">'
                     f'<text x="{KEY_X+14}" y="{y:.1f}" fill="{ACCENT}" font-size="12" text-decoration="underline">{txt}</text>'
                     f'</a>')
        else:
            txt = esc(txt_raw)
            inner = (f'<circle cx="{KEY_X+3}" cy="{y-4:.1f}" r="2.5" fill="{GREEN}"/>'
